...
open class NormalHeaderAnimator: UIView, CRRefreshProtocol {
    ...
    open func refreshBegin(view: CRRefreshComponent) {
        UIImpactFeedbackGenerator(style: UIImpactFeedbackGenerator.FeedbackStyle.medium).impactOccurred()
    ...
    open override func layoutSubviews() {
        ...
        UIView.performWithoutAnimation {
            ...
            indicatorView.center = .init(x: titleLabel.frame.origin.x - 20.0, y: h / 2.0)
            imageView.frame = CGRect.init(x: (w - 20) / 2, y: (h - 20.0) / 2.0, width: 20, height: 20)
        }
    }
